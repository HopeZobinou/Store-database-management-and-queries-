...
class Submission:
    ...
    def __clear_views(self):
        ...
        try:
            ...
            #[_] Still need to reproduce this one, temporary fix
            print('\n\n\nPlease let the professor know you recieved this message! IndexError in __clear_views()\n\n\n')
            return
    ...
    def as_df_short_string(self, 
        ...
        if len(self.rows) > max_len:
            df_head = pd.DataFrame(data = self.rows[0:head_tail_len],
                                   columns = self.cols
                                   ).map(lambda y: y[:max_str_len] if isinstance(y, str) else y)
            ...
            df_str = pd.DataFrame(data=self.rows, 
                                  columns=self.cols
                                  ).map(lambda y: y[:max_str_len] if isinstance(y, str) else y).to_string(index=False)
